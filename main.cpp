...
int BackTrack(string& rna_solution, string& structure_solution, string& rna_seq, AllTables& alltables, vector<int>& con_seq){
    ...
    // std::cerr << "max score: " << maxscore << std::endl;
    ...
    while ( !stk.empty() ) {
        ...
        int k, nuck, nucl;
        /* cout << "j: " << j << ", i: " << i << endl;
        std::cout << -(maxscore - state.score)/100 << " (" << -(state.score/100.0) << ")" << std::endl;
        // std::cout << std::endl; 
...
int main(int argc, char** argv){
    ...
    std::vector<std::string> rna_seq_list, ami_seq_list;
    std::vector<std::vector<int>> con_seq_list;
    std::vector<float> cai_vector;

    bool isRNAfile = false;
    initialize();
    beamsize = 0;
    lambda = 0;
    ...
    if(input.cmdOptionExists("-h")){
        std::cout << "./main [-f] [file_path] [-b] [beam_size] [-l] [lambda] [-cai] [cai_file_path]" << endl;
        return 0;
    }
    ...
    if (!beamsize_.empty()){
        ...
        std::cout << "beam size: " << beamsize << endl;
    }

    const std::string &lambda_ = input.getCmdOption("-l");
    if (!lambda_.empty()){
        std::cout << "lambda: " << lambda_ << endl;
        lambda = stof(lambda_);
    ...
    if (!file.empty()){
        ...
        const std::string &RNAfile = input.getCmdOption("-RNAf");
        if (!RNAfile.empty()){
            fasta_file.open(RNAfile);
            std::cout << "input file: " << RNAfile << endl;
            while (fasta_file >> seq){
                if (seq.empty()) continue;
                else if (seq[0] == '>' or seq[0] == ';'){
                    if (!rna_seq.empty())
    ...
    if(!isRNAfile){
        ...
        for(int i = 0; i < ami_seq_list.size(); i++){
            ...
            if(lambda)
                ...
                std::cerr << "MFE: " << -(maxscore/100.0) << std::endl;
        ...
        for(int i = 0; i < rna_solution.size(); ++i){
            if(i%3 == 2){
                std::string codon_ = "---";
                codon_[0] = rna_solution[i - 2];
                codon_[1] = rna_solution[i - 1];
                codon_[2] = rna_solution[i];
                int ami_ID = i/3;
                char ami_ = ami_seq[ami_ID];
                int last_nuc = getLastExtendedNuc(ami_ ,codon_);
                int weight_ = CodonSetCAIMap[ami_][last_nuc];
                if(weight_ != 0){
                    cout << codon_ << endl;
                    cout << ami_ << endl;
                    cout << "--" << endl;
